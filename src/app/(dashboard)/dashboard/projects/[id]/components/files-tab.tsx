...
import type { ProjectFile, Campaign } from '../types'
...
export function FilesTab({ files, campaigns }: FilesTabProps) {
  ...
  const filtered = useMemo(() => {
    return files.filter((f) => {
      ...
      if (campaignFilter !== 'all' && f.campaign?.id !== campaignFilter) return false
      return true
    })
  }, [files, typeFilter, campaignFilter])
                ...
                return (
                  <TableRow key={f.id}>
                    ...
                    <TableCell>
                      {f.campaign ? (
                        <Link
                          href={`/dashboard/campaigns/${f.campaign.id}`}
